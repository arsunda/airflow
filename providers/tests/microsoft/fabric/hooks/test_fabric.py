# Licensed to the Apache Software Foundation (ASF) under one
# or more contributor license agreements.  See the NOTICE file
# distributed with this work for additional information
# regarding copyright ownership.  The ASF licenses this file
# to you under the Apache License, Version 2.0 (the
# "License"); you may not use this file except in compliance
# with the License.  You may obtain a copy of the License at
#
#   http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing,
# software distributed under the License is distributed on an
# "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
# KIND, either express or implied.  See the License for the
# specific language governing permissions and limitations
# under the License.

from __future__ import annotations

import time
from unittest import mock
from unittest.mock import MagicMock, patch

import pytest
import requests
from tenacity import RetryError

from airflow.models.connection import Connection
from airflow.providers.microsoft.fabric.hooks.fabric import (
    FabricRunItemException,
    FabricRunItemStatus,
    MSFabricAsyncHook,
    MSFabricHook,
)

DEFAULT_FABRIC_CONNECTION = "fabric_default"
WORKSPACE_ID = "workspace_id"
ITEM_ID = "item_id"
ITEM_RUN_ID = "item_run_id"
ITEM_RUN_LOCATION = f"https://api.fabric.microsoft.com/v1/workspaces/{WORKSPACE_ID}/items/{ITEM_ID}/jobs/instances/{ITEM_RUN_ID}"
BASE_URL = "https://api.fabric.microsoft.com"
API_VERSION = "v1"
JOB_TYPE = "RunNotebook"
MODULE = "airflow.providers.microsoft.fabric.hooks.fabric"
JOB_PARAMS = None


@pytest.fixture(autouse=True)
def setup_connections(create_mock_connection):
    create_mock_connection(
        Connection(
            conn_id=DEFAULT_FABRIC_CONNECTION,
            conn_type="fabric",
            login="clientId",
            password="userAcessToken",
            extra={
                "tenantId": "tenantId",
            },
        )
    )


@pytest.fixture
def fabric_hook():
    client = MSFabricHook(fabric_conn_id=DEFAULT_FABRIC_CONNECTION)
    return client


@pytest.fixture
def get_token(fabric_hook):
    fabric_hook._get_token = MagicMock(return_value="access_token")
    return fabric_hook._get_token


def test_get_headers(get_token, fabric_hook):
    headers = fabric_hook.get_headers()
    assert isinstance(headers, dict)
    assert "Authorization" in headers
    assert headers["Authorization"] == "Bearer access_token"


def test_get_item_run_details_success(fabric_hook, get_token, mocker):
    # Mock response for successful response from _send_request
    response = MagicMock()
    response.ok = True
    response.json.return_value = {"status": FabricRunItemStatus.COMPLETED}

    mocker.patch.object(fabric_hook, "_send_request", return_value=response)
    mocker.patch.object(fabric_hook, "get_headers", return_value={"Authorization": "Bearer access_token"})

    result = fabric_hook.get_item_run_details(location=ITEM_RUN_LOCATION)

    assert result == {"status": FabricRunItemStatus.COMPLETED}
    fabric_hook.get_headers.assert_called_once()
    fabric_hook._send_request.assert_called_once_with(
        "GET", ITEM_RUN_LOCATION, headers={"Authorization": f"Bearer {get_token.return_value}"}
    )


def test_get_item_run_details_failure(fabric_hook, get_token, mocker):
    # Mock response for failed response from _send_request
    response = MagicMock()
    response.ok = False
    response.raise_for_status.side_effect = requests.exceptions.HTTPError("Error")

    mocker.patch.object(fabric_hook, "_send_request", return_value=response)
    mocker.patch.object(fabric_hook, "get_headers", return_value={"Authorization": "Bearer access_token"})

    with pytest.raises(RetryError, match="Error"):
        fabric_hook.get_item_run_details(location=ITEM_RUN_LOCATION)

    assert fabric_hook._send_request.call_count == 5
    fabric_hook.get_headers.call_count == 5

    fabric_hook._send_request.assert_any_call(
        "GET", ITEM_RUN_LOCATION, headers={"Authorization": f"Bearer {get_token.return_value}"}
    )


@patch(f"{MODULE}.MSFabricHook._send_request")
def test_get_item_details(mock_send_request, fabric_hook, get_token):
    fabric_hook.get_item_details(WORKSPACE_ID, ITEM_ID)
    expected_url = f"{BASE_URL}/{API_VERSION}/workspaces/{WORKSPACE_ID}/items/{ITEM_ID}"
    mock_send_request.assert_called_once_with(
        "GET", expected_url, headers={"Authorization": f"Bearer {get_token.return_value}"}
    )


@patch(f"{MODULE}.MSFabricHook._send_request")
def test_run_fabric_item(mock_send_request, fabric_hook, get_token):
    fabric_hook.run_fabric_item(WORKSPACE_ID, ITEM_ID, JOB_TYPE)
    expected_url = f"{BASE_URL}/{API_VERSION}/workspaces/{WORKSPACE_ID}/items/{ITEM_ID}/jobs/instances?jobType={JOB_TYPE}"
    mock_send_request.assert_called_once_with(
        "POST", expected_url, headers={"Authorization": f"Bearer {get_token.return_value}"}, json={}
    )


@patch(f"{MODULE}.MSFabricHook._send_request")
def test_send_request(mock_send_request, fabric_hook: MSFabricHook):
    request_type = "GET"
    url = "https://api.fabric.microsoft.com/test"
    fabric_hook._send_request(request_type, url)
    mock_send_request.assert_called_once_with(request_type, url)


@patch(f"{MODULE}.MSFabricHook._send_request")
def test_send_request_with_custom_headers(mock_send_request, get_token, fabric_hook):
    request_type = "GET"
    url = "https://api.fabric.microsoft.com/test"
    headers = {"Content-Type": "application/json", "Authorization": f"Bearer {get_token.return_value}"}
    fabric_hook._send_request(request_type, url, headers=headers)
    mock_send_request.assert_called_once_with(
        request_type,
        url,
        headers={"Content-Type": "application/json", "Authorization": "Bearer access_token"},
    )


@pytest.fixture
def fabric_async_hook():
    client = MSFabricAsyncHook(fabric_conn_id=DEFAULT_FABRIC_CONNECTION)
    return client


@pytest.mark.asyncio
async def test_async_get_cached_token(fabric_async_hook):
    # Mocking the async dependencies
    mock_cached_access_token = {"access_token": "valid_token", "expiry_time": time.time() + 1000}

    fabric_async_hook.cached_access_token = mock_cached_access_token
    token = await fabric_async_hook._async_get_token()

    # Check that the correct token was returned
    assert token == "valid_token"


@pytest.mark.asyncio
async def test_async_get_no_cached_token(fabric_async_hook):
    # Mocking the situation where there is no cached token
    fabric_async_hook.cached_access_token = {}

    # Mocking the connection object and its attributes
    mock_connection = Connection(
        login="clientId", password="refresh_token", extra={"tenantId": "tenant_123", "scopes": "scope_123"}
    )

    # Mocking the response from async_send_request (API response)
    mock_response = {"access_token": "new_token", "refresh_token": "new_refresh_token", "expires_in": 3600}

    with patch.object(fabric_async_hook, "async_send_request", return_value=mock_response):
        with (
            patch.object(fabric_async_hook, "get_connection", return_value=mock_connection),
            patch(f"{MODULE}.update_conn"),
        ):
            # Call the function under test
            token = await fabric_async_hook._async_get_token()

    assert token == "new_token"
    assert fabric_async_hook.cached_access_token["access_token"] == "new_token"
    assert fabric_async_hook.cached_access_token["expiry_time"] == pytest.approx(time.time() + 3600)


@pytest.mark.asyncio
async def test_async_get_token_failure(fabric_async_hook):
    fabric_async_hook.cached_access_token = {}

    # Mocking the methods that are used within _async_get_token
    mock_connection = Connection(
        login="clientId", password="refresh_token", extra={"tenantId": "tenant_123", "scopes": "scope_123"}
    )

    # Mocking the API response to simulate failure (missing access_token or refresh_token)
    mock_response = {"access_token": None, "refresh_token": None}
    with patch.object(fabric_async_hook, "async_send_request", return_value=mock_response):
        with patch.object(fabric_async_hook, "get_connection", return_value=mock_connection):
            with pytest.raises(
                FabricRunItemException, match="Failed to obtain access or refresh token from API."
            ):
                await fabric_async_hook._async_get_token()


@pytest.mark.asyncio
async def test_async_get_headers(fabric_async_hook):
    # Mocking the _async_get_token method to return a fixed token
    with patch.object(fabric_async_hook, "_async_get_token", return_value="mocked_token"):
        headers = await fabric_async_hook.async_get_headers()

    # Verify that the headers are correct
    assert headers == {"Authorization": "Bearer mocked_token"}


@pytest.mark.asyncio
@mock.patch(f"{MODULE}.MSFabricAsyncHook._async_get_token", return_value="access_token")
async def test_async_get_item_run_details_success(mock_get_token, fabric_async_hook, mocker):
    # Mock response for successful response from _send_request
    response = MagicMock()
    response.ok = True
    response.json.return_value = {"status": FabricRunItemStatus.COMPLETED}

    mocker.patch.object(
        fabric_async_hook,
        "async_get_headers",
        return_value={"Authorization": f"Bearer {mock_get_token.return_value}"},
    )
    mocker.patch.object(fabric_async_hook, "async_send_request", return_value=response.json.return_value)

    expected_url = (
        f"{BASE_URL}/{API_VERSION}/workspaces/{WORKSPACE_ID}/items/{ITEM_ID}/jobs/instances/{ITEM_RUN_ID}"
    )
    result = await fabric_async_hook.async_get_item_run_details(
        workspace_id=WORKSPACE_ID, item_id=ITEM_ID, item_run_id=ITEM_RUN_ID
    )

    assert result == {"status": FabricRunItemStatus.COMPLETED}
    fabric_async_hook.async_get_headers.assert_called_once()
    fabric_async_hook.async_send_request.assert_called_once_with(
        "GET", expected_url, headers={"Authorization": "Bearer access_token"}
    )


@pytest.mark.asyncio
@mock.patch(f"{MODULE}.MSFabricAsyncHook.async_send_request")
@mock.patch(f"{MODULE}.MSFabricAsyncHook._async_get_token", return_value="access_token")
async def test_async_cancel_item_run(mock_get_token, mock_send_request, fabric_async_hook):
    await fabric_async_hook.async_cancel_item_run(WORKSPACE_ID, ITEM_ID, ITEM_RUN_ID)
    expected_url = f"{BASE_URL}/{API_VERSION}/workspaces/{WORKSPACE_ID}/items/{ITEM_ID}/jobs/instances/{ITEM_RUN_ID}/cancel"
    mock_send_request.assert_called_once_with(
        "POST", expected_url, headers={"Authorization": "Bearer access_token"}
    )
